 ...
 */
public class BulkLoadJob extends LoadJob {

    private static final Logger LOG = LogManager.getLogger(org.apache.doris.load.loadv2.BrokerLoadJob.class);
    ...
    private BulkStorageDesc bulkDesc;
    ...
    public BulkLoadJob(long dbId, String label, BulkStorageDesc bulkDesc,
                         OriginStatement originStmt, UserIdentity userInfo)
            throws MetaNotFoundException {
        super(EtlJobType.BROKER, dbId, label);
     ...
     */
    @Override
    public void onTaskFinished(aTaskAttachment attachment) {
    ...
    private void createLoadingTask(Database db, BrokerPendingTaskAttachment attachment) throws UserException {
        ...
        try {
            for (Map.Entry<FileGroupAggKey, List<BrokerFileGroup>> entry
                ...
                LoadLoadingTask task = new LoadLoadingTask(db, table, bulkDesc,
    ...
    protected String getResourceName() {
        StorageBackend.StorageType storageType = bulkDesc.getStorageType();
        if (storageType == StorageBackend.StorageType.BROKER) {
            return bulkDesc.getName();
        } else if (storageType == StorageBackend.StorageType.S3) {
            return Optional.ofNullable(bulkDesc.getProperties())
